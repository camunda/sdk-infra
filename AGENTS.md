...
| `sdk-breaking-change-guard.yml` | Fail PR CI when commits contain `BREAKING CHANGE:`/`BREAKING-CHANGE:` notes in body/footer that would trigger a major version bump; bypassed by the `breaking-change-approved` label |
