...
| `sdk-breaking-change-guard.yml` | Fail PR CI when commits contain breaking change markers (`BREAKING CHANGE:` footer or `!` after type) that would trigger a major version bump; bypassed by the `breaking-change-approved` label |
